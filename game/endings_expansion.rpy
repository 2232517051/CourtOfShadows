...
label ending_peoples_epilogue:
    ...
    "第二天，东村照旧来争水渠，账房照旧催欠账。散会后，村民赶回去收麦，不必先问哪支军队会经过田里。"
    ...
    if elena_romance:
        ...
        "五年来，她一直留在艾登堡。不是王后的眼线，也不是暗百合的刀，只以自己的名字生活。"
        if rel_elena >= 50:
            hide elena_img
            $ hide_all_chars("player_char_img")
            show player_char_img at left with dissolve
        ...
        $ hide_all_chars("ingrid_img")
        show ingrid_img at left with dissolve
        "英格丽从花园门外进来，臂弯里夹着北境议会刚送到的盐价表。五年前，她以一纸盟约来到这里；如今她仍是你的妻子，这座城堡也是她的家。"
        if marriage_warm:
            ...
            "你们各守一间书房，各自履行盟约。没有人把它误认作爱情，但晚饭仍摆在同一张餐桌上。"
        hide ingrid_img with dissolve
    elif corsair_romance:
        $ hide_all_chars()
        scene bg study with dissolve
        $ hide_all_chars()
        if southern_outcome == "fall":
            "书房窗边有一块颜色略浅的木纹。那截渡鸦号旧绳头原先压在那里；卖掉潮汐港后，那里一直空着。"
            "五年里，赛琳没有捎来一个字。南边的商人偶尔提起一条换了涂装的旧船，你没有让他们追问。"
        else:
        ...
        scene bg palace_garden with dissolve
        $ hide_all_chars()
        "花园里只剩奥尔德里克翻土的声音。你的房门里没有人在等，餐桌另一边也没有固定的位置。"
        "大厅里仍有人等着议水渠和面粉。明早，东村要带田界图来，西村也约了账房核对公粮。"
    ...
    player "哪条路常被抢，哪个村去年少收了多少，都要先查清楚。账对不上，后面的事就无从谈起。"
    ...
    if dark_lily_joined:
        "暗百合内部为是否公开身份和档案争论了多年。一部分成员成为公开监察员，另一部分离开；继任者没有强迫所有人接受同一种答案。"
    elif dark_lily_destroyed:
