...
label ending_sea_epilogue:
    ...
    if corsair_romance:
        ...
        "她说这话的时候看着海面，像在说一件和天气一样确定的事。"

        if rel_corsair >= 80:
            "那天晚上你睡不着。她也没睡。你伸手握住她编绳的手——绳结硌在掌心中间，不柔软，但很实在。"
        ...
        "你从一个港换到另一个港，靠修补铁器和打零工过活。船上的日子单调得像一条直线。"
        ...
        "有一天遇到一个北方来的皮货商。你鼓起勇气问了一句——艾登堡还在吗？"
...
label ending_truth_epilogue:
    ...
    if rel_captain >= 60:
        "你把城防的印信正式交到他手上。十二年前他从西境战场活着回来，如今整座城的墙都归他守。"
...
label ending_peoples_epilogue:
    ...
    if rel_elena >= 50:
        ...
        "但她还在这里。"
        "在那场风波之后，在一切落定之后——她选择留在艾登堡的这一畦菜地旁边。"
    ...
    player "兵多没用。你得让百姓觉得这片地方值得守。"

    player "当百姓信任你的时候，他们不会偷税——因为他们知道税收是为了他们自己。"

    player "你家伯爵要是真想学，先把路修好、贼赶走。剩下的，人心替他办。"
    ...
    if built_school:
        ...
        "艾登堡没有乱。第二天，铁匠的锤声照常响起，孩子们照常在巷子里追跑，面包照常出炉。"
    ...
    if baron_peace_path:
        ...
        "他在议会里的发言务实而老练——毕竟管理领地的经验不会因为换了头衔就消失。"
    else:
    ...
    if dark_lily_joined:
        "你给了他们一条路：不再藏在地下，改去替百姓盯着当权的人。做的事差不多，只是可以见光了。"
        "有些人适应了，有些人离开了。但那个从阴影中走出来的组织，第一次站在了阳光下。"
    elif dark_lily_destroyed:
        "它早已不存在了。人民不需要知道它曾经存在过。"
    else:
        "没有人下令解散它——它自己消失了。在一个人人都可以发声的世界里，秘密结社变得多余。"
        "最后一批成员默默摘下了徽章，走进了人群中。他们不再是暗百合——也不需要是了。"

    "雷恩把守军拆了——改成轮值，农闲时练矛，有事时全城皆兵，不再养一支只听领主号令的队伍。"

    "有人劝他留个正规营「以防万一」。他说：「万一来了，全城都是兵。」"

    "如今他在校场边开了间小铺子，卖磨好的农具。孩子们还是叫他队长。"
...
label ending_side_characters_fate:
    ...
    ## 可用"艾德蒙", arrested/negotiated 只能"叔叔/首领"(那两条线没报过名)。
    if ch3_cult_leader_fate == "captured":
        ...
        if _fate_mood == "light":
            "温和派彻底回归正统之后，你的叔叔就从所有消息里消失了。"

            "有人说在南方的修道院见过一个满脸疤痕的男人。你没有派人去查。"
        elif _fate_mood == "neutral":
            "你的叔叔再没有出现过。温和派的人不提他，你也不问。"

            "铁刺派旧据点变成了仓库和马厩。你路过的时候没多看过一眼。"
        else:
            "有传言说你的叔叔还活着。也有传言说他死在了路上。两种你都听过。"
