...
label ending_peoples_epilogue:
    ...
    "田边堆着捆好的麦秆，果园里有人把最后几筐苹果搬上车。"
    "广场支起长桌，新出炉的面包和苹果酒一份份送过来。"
    ...
    "平日里，田界仍要重新丈量，账房仍要催欠账。村民争完便赶回去收麦，不必先问哪支军队会经过田里。"
    ...
    if built_clinic:
        ...
        "笑声散去后，玛格丽特把门外等候的第一个病人叫进新馆。刚才起哄的人收好酒杯，给担架让开了路。"
    ...
    if elena_romance:
        $ hide_all_chars()
        scene bg village with dissolve
        "广场边，孩子们正给苹果木牌系彩带。艾琳娜坐在台阶上，用小刀削掉多余的线头。"
        $ hide_all_chars("elena_img")
        show elena_img at left with dissolve
        elena "躲到花园还不够，还想躲到人群最后一排？"
        if rel_elena >= 50:
            hide elena_img
            $ hide_all_chars("player_char_img")
            show player_char_img at left with dissolve
            ...
            $ hide_all_chars("elena_img")
            show elena_img at left with dissolve
            "她合上小刀，把酒杯塞给路过的少年，牵住你的手。"
            elena "这个借口，留到跳完再说。"
        else:
            elena "孩子们还缺一个领舞的人。您去吧。"
            "她没有挽住你，只替你拍掉肩头的草屑。一个孩子的彩带松了，她转身重新系好；你独自走向舞圈。"
        hide elena_img with dissolve
    elif marriage_route:
        $ hide_all_chars()
        scene bg castle_exterior with dissolve
        $ hide_all_chars("ingrid_img")
        show ingrid_img at left with dissolve
        "城堡门前，英格丽正把北境蜂蜜饼分给返家的孩子。五年前，她因盟约来到艾登堡；如今她仍是你的妻子，这座城堡也是她的家。"
        if marriage_warm:
            ingrid "下一支舞快开始了。上次你踩了我三次，今晚最多准一次。"
            "散场后，她挽着你的手穿过城门。门厅里，两双沾了泥的鞋并排留在地毯外。"
        else:
            ingrid "北境的客人已经离城。我按礼单一项项送完了。您可以放心。"
            "回城时，你们一前一后穿过同一扇门。她走向东侧楼梯，厨房照惯例把晚饭摆在长桌两端。"
        hide ingrid_img with dissolve
    elif corsair_romance:
        $ hide_all_chars()
        scene bg village with dissolve
    ...
    $ hide_all_chars("player_char_img")
    show player_char_img at left with dissolve
    player "先把你们近三年的税账和各村欠粮名单拿出来。"

    player "哪段商路常有劫匪出没，哪个村去年少收了多少，都要先查清楚。账对不上，后面的事就无从谈起。"

    player "艾登堡把税账贴到广场以后，抱怨没有消失。账房得逐笔解释；解释不清的，就把错账改掉。"
    ...
    $ hide_all_chars("player_char_img")
    show player_char_img at left with dissolve
    player "会有人藏粮，也会有人不肯签字。艾登堡的两个村子为一条水渠吵了半个月。"

    player "先让双方把地界、出工人数和用水时辰摊在桌上。查出谁说了假话，再按账追缴；不能因为争得凶就派兵抓一村人。"

    player "你家伯爵要是真想学，先把路修好、贼赶走，再让百姓看见每一笔税花在了哪里。"

    $ hide_all_chars()
    "管事站起身，深深鞠了一躬。"

    "\"多谢领主大人赐教。我会一字不差地转告我家伯爵。\""
    ...
    if built_school:
        ...
        "\"[player_name]，我家闺女在教堂跟修士认了字，会写自己名字了！\""
    ...
    "\"往年新麦先拿去抵租。今年我留了一小袋，够烤三炉。您手里是第一炉。\""

    "你咬了一口面包。"

    "很普通的面包。粗面粉，没有黄油，没有糖。"
    "你吃完一半，把剩下的一半用手帕包好。老太太坐回椅子上，又把两块面包递给刚收摊的提琴手。"

    "天黑前，你带着那半块面包回到城堡。大厅里已经摆好东村和西村的水渠图，账房在旁边等着。"

    "以后每个丰收节，你仍会经过这条街。有些年份桌上摆满苹果，有些年份连酒也要限量；大厅里的争吵从来没有断过。"


    ## —— 第五幕：晚年与远讯 ——

    "你的白发一年比一年多。艾登堡广场上的账册也一年比一年厚。"

    "偶尔有远方的商队路过，捎来一些王都的消息。"

    "关于王后的消息越来越少。有人说她病了，有人说她累了，有人说她的派系散了。"

    "最早把提案送进王都议会的，是几座城镇行会、村社推举的代表，以及少数愿意公开账目的小领主。"
    "第一份提案要求各领地公开地租与税款去向，并准许村社代表列席地方议事。贵族席以『动摇旧律』为由否决了它。"
    "负责抄印提案、在市集召集联署的人被控『煽动抗税』。有人入狱，有人退回自己的领地。"
    "两年后，另一批行会把提案拆成两份：一份只要求公开账目，一份要求村社有列席权；艾登堡五年的收支抄本被订在两份提案后面。"
    "几个急着清理旧债的领地先接受了公开账目，查出仓吏和包税人的亏空。村社代表列席议事的提案仍被搁置；另一些领主在收成好转后撤下公示牌，赶走村社代表，拒缴地租的冲突又起。"
    ...
    "你是在一个平凡的春日清晨走的。"
    "没有号角，没有仪仗。遗嘱只写了一件后事：在领主常去的苹果树下，埋一块无字的石头。"
    ...
    "很多年后，一位写史的人在王都议会的驳回卷宗里找到一份艾登堡五年账册。旁边还夹着被捕者名单和几份重新递交的提案。"
    "有些城镇公开账目后，查出了官吏侵吞的粮款；有些领主在歉收后撤下公示牌，村社代表为此停工，卫兵随后抓走了带头的人。"
    "最初否决提案的贵族席后来换了几轮人。每一轮都删掉一些条文，也被地方议会和行会逼着补回另一些。"
    "艾登堡的记录同样有反复：接任者废掉过议事规矩，村社又把它们重新写回章程。"
    ...
    if baron_peace_path:
        ...
        "男爵组织过抵抗，但很快战败。他失去头衔和领地；他的旧部没有立刻消失，边地冲突又延续了数年。"

    if prince_ally:
        "弗雷德里克化名去了南方教书。他偶尔写文章谈责任，从不署上王族旧姓。"
    else:
        "弗雷德里克流亡海外。复辟者用他的名字募钱、招兵；几年后，捐款越来越少，边境上的骚乱却没有立刻结束。"

    if dark_lily_joined:
        "暗百合内部为是否公开身份和档案争论了多年。一部分成员成为公开监察员；不愿公开的人可以离开，不再参与监察。地方议会要求在任监察员公开经费和案卷。"
    elif dark_lily_destroyed:
    ...
    centered "「苹果树下的石头没有刻名字。」"
    centered "「广场上的账册每月照旧更换。」"
    centered "「有人把旧账抄走，带回了自己的村子。」"
