...
label chapter3_start:
    call new_run_bootstrap from _call_new_run_bootstrap_chapter3
...
label ch3_strange_signs:
    ...
    if dark_lily_exists_known:
        ...
        captain "那就好说了。这两天又多了好几处新的，不是巧合。"
    else:
        player "这是什么意思？"
    ...
    menu:
        "召集附近领主家臣联合搜山" if power >= 50:
            ...
            "你向友好领主回了重礼，并许诺未来三年内对方有需要，可以反过来求援。"
...
label ch3_after_field_choice:
    ...
    "奥尔德里克说话还是那副什么都知道的口气。但他嘴角的笑意没撑多久就垮了。"
...
label ch3_fathers_study:
    ...
    "耐心快磨光了，但你想起奥尔德里克的话——你父亲藏东西的手段不是一般人能想到的。"
    ...
    "『据说它来自南方深山某种紫色苔藓的根须，采集极难，提炼更难。能稳定制出成品的工坊，整个王国不超过三家。』"

    "『但暗百合的炼金术士验过先王的遗物——有残留。』"
    ...
    "『第十四年，秋。我派了一个亲信去王都调查暮色之露的来源。他查到了——毒药是教会的炼金术士格温制造的。格温是费雷恩大主教的人。一切线索都指向教会——但教会是主谋还是工具？我还不确定。』"

    "『第十四年，冬。我的人在王都被人发现了。他们杀了他，伪装成意外。我失去了一个忠诚的仆人。我开始意识到——我面对的不是一两个人，而是一张大网——从宫廷到教会到各地贵族，环环相扣。我一个人……根本无法对抗。』"

    "越往后，父亲的笔调越沉。"

    "『第十五年。马修斯主教开始频繁来访。他的眼神让我不安。我怀疑他已经知道我在调查什么。教会在这件事中扮演了什么角色？他们是同谋，还是被利用了？马修斯和费雷恩不一样——他似乎有自己的良心。但我不知道他的良心还能撑多久。』"
...
label ch3_dark_lily_clues:
    ...
    "文字的书写方式很古老——你认出了一些字母，但排列方式与当世通行的文字截然不同。"

    "少说两百年了。要完全破译，得找个通晓古文的学者。"
    ...
    "第三幅壁画最让你震动——一个人影站在暗处，手中握着一把匕首，刃上沾着毒液，尖端指向王冠。"
    ...
    "你叫来了一个信得过的侍女，让她帮你找一套普通人的衣服——粗布衬衫、麻布裤子、一件带兜帽的旧斗篷。"
    ...
    "你注意到她的围裙上绣着一朵百合花——不是倒置的，但直觉告诉你这不是巧合。"
    ...
    menu:
        ...
        "为了保护领地和领民":
            ...
            $ hide_all_chars("lily_root_img")
            show lily_root_img at left with dissolve
            lily_interviewer "领民……比多数找上门来的人说得实在。"
    ...
    menu:
        ...
        "我信任奥尔德里克和雷恩":
            ...
            $ hide_all_chars("lily_root_img")
            show lily_root_img at left with dissolve
            lily_interviewer "有限度的信任……活到我这把年纪的人，都是这么过来的。"
...
label ch3_no_passage:
    ...
    $ hide_all_chars("lily_root_img")
    show lily_root_img at left with dissolve
    lily_root "你父亲是我们最重要的盟友之一。他走了以后……很多事再也接不上了。"
...
label ch3_study_return_no_passage:
    ...
    ## 总部可选化(2026-07-24): 原本无条件跳进总部, 现在改道赴约之夜的抉择门
    jump ch3_hq_gate
...
label ch3_tunnel_exploration:
    ...
    ## ============================================================
    ## 赴约之夜的抉择 —— 总部可选化 (2026-07-24)
    ## 两条路线在此汇合: 密道路线从上面顺序落入; 无密道路线由 :2036 改道跳入。
    ## 「不去」的玩家接回 ch3_elena_secret(其 else 分支不引用总部, "第二天清晨"的时间锚
    ## 在爽约当夜之后依然成立), 之后 chapter3_expansion:968 的守卫因 visited=False 放行,
    ## 备用路线(渗透铁刺派集会+艾德蒙揭晓)自然可达。
    ## 注意: 门的文案对两种状态都要成立 —— 多数人持正式邀约(lily_trial_passed),
    ## "假装普通客人"支线的玩家是自己蹲点摸到的地点、没有邀约。
    ## ============================================================
...
label ch3_hq_gate:
    ...
    "你在书房里坐到天黑。蜡烛没点，桌上摊着父亲的日记，字迹已经看不清了。"

    "暗百合的据点——你知道在哪。今晚赶过去，也许能从首领嘴里问出父亲到底是怎么死的。"

    "但父亲当年也是一步步走进去的。走进一个他没看清的局，再没出来过。"
    ...
    menu:
        ...
        "不去":
            $ log_decision("第三章", "没有赴暗百合之约——用自己的方式查")
            ## 第四章开场回顾的动态行: 默认值是"加入还是捣毁"(赴约语义, 老档兼容),
            ## 爽约在此换成不去版(文案任务5)
            $ ch3_recap_lily_line = "你绕开了暗百合——自己查下去，瓦解了铁刺派。"

            "你没有去。"

            "不是怕。是你不打算把父亲的死因押在一群来路不明的人身上。"

            "日记就在桌上，暮色之露的样本锁在抽屉里。该查的名字你记得住。不用非得从别人嘴里听。"

            "你关上窗，上了闩，在黑暗里躺下。"

            "夜很长。没人来敲门，没有蒙面人翻墙进来，什么也没发生。"

            "你等过一阵——等一封信，一句威胁，或者更不露痕迹的东西。什么都没有。"

            "好像他们根本不在乎你来不来。"

            "天快亮的时候，你才闭上眼。"

            jump ch3_elena_secret
...
label ch3_dark_lily_hq:
    ...
    ## 总部可选化: 原开头"三天后。子时。"的时间跳跃已由 ch3_hq_gate 的定场承担
    ## (门就发生在赴约当夜), 保留时刻、删去重复的跳跃。
    "子时。"
    ...
    $ hide_all_chars("lily_master_img")
    show lily_master_img at left with dissolve
    lily_master "是。你父亲还在世时，我就一直在等——等他的继承人自己走进来。"
    ...
    lily_master "所以我们不需要遍布每一寸土地。每一处关节上有一个人，就够了。"
    ...
    $ hide_all_chars("lily_master_img")
    show lily_master_img at left with dissolve
    lily_master "你父亲身边的酒窖管事——汉斯。跟了卡尔大人二十年的老人。"
...
label ch3_truth_emerges:
    ...
    if secret_passage_found:
        "密道中发现的卷轴——暗百合两百年的历史。"
    if not ch3_dark_lily_visited:
        ## 总部可选化: 爽约的玩家从未拿到首领证词, 证据只有日记与毒药样本
        "父亲的日记与暮色之露样本——先王的死并非天命。毒药“暮色之露”出自教会炼金术士格温之手。"
    elif not testament_forged_known:
    ...
    if not ferein_role_known:
        ...
        "你用红线连接了所有已知的关键节点。图上没有断头了——每条线都合上了。"
    ...
    if testament_original_obtained:
        ...
        if ch3_dark_lily_visited:
            "影的话在你耳边回响：「有了原始遗诏，真相才有了无法辩驳的锋芒。」"
        else:
            "你想起父亲在日记里写的：『证据还在。只要拿到手——他们编的那套说辞，就全完了。』原始遗诏——就是那份证据。"
...
label ch3_bishop_take_to_vault:
    ...
    "他的脸上看不出罪恶的痕迹。画师笔下的费雷恩，像任何一位虔诚的教士。"
    ...
    "你捏着羊皮纸的边角，指尖发白。二十年前被掩埋的真相，就攥在你手心里。"
...
label ch3_critical_choice:
    ...
    "雷恩的手不自觉地握紧了剑柄。半晌，他脸上的震惊沉下去，只剩一层铁青。"
    ...
    aldric "如果我们贸然公布，她会说遗诏是伪造的，然后以叛逆罪名讨伐我们。"
    ...
    $ hide_all_chars("aldric_img")
    show aldric_img at left with dissolve
    aldric "不管证据够不够，你现在危险得很。王后迟早会发现你在调查。"
...
label ch3_church_intervention:
    ...
    menu:
        ...
        "先见见这个审判官，摸清他的底细":
            ...
            $ hide_all_chars("bishop_img")
            show bishop_img at left with dissolve
            bishop "大人，我见过他审人。什么都不问，就盯着你看，等你自己说漏嘴。"
            hide bishop_img
            $ hide_all_chars("player_char_img")
            show player_char_img at left with dissolve
    ...
    "王后不再满足于让男爵暗中操控了——她动用了教会。"

    "教会审判庭的权力几乎不受限制。在教会的领地上——包括教堂和修道院——审判官的权力甚至大于领主。"

    "你无法用武力阻止搜查。强行对抗教会审判，只会给王后一个讨伐的借口——「蔑视神权」。"
    ...
    $ hide_all_chars("captain_img")
    show captain_img at left with dissolve
    captain "审判官？那些人能搜查任何地方——教堂、修道院，甚至领主的城堡。谁也拦不住。"
...
label ch3_end:
    ...
    ## ── 3.9.3 格伦瓦德对峙(晨曦反馈: 男爵作为前期小boss) — 盟约线内部跳过 ──
    call ch3_baron_clash from _call_ch3_baron_clash
