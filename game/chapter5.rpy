...
label chapter5_start:
    call new_run_bootstrap from _call_new_run_bootstrap_chapter5
...
label ch5_war_clouds:
    ...
    menu:
        "向他们承诺会保护所有人":
            ...
            $ hide_all_chars()
            "老农磕头磕得额头红了，铁匠在一旁站得笔直。那妇人抱着孩子，手终于不抖了。"
    ...
    if spy_network:
        ...
        $ hide_all_chars()
        "没有间谍网络的精准情报，你只能依靠老兵的直觉和地形的优势。够不够用，打起来才知道。"
...
label ch5_factions_move:
    ...
    elif iron_thorn_controlled:
        ## 艾德蒙回响F(接管线): 铁刺网经中间人链回传敌情。艾琳娜不在知情圈,
        ## 她只能评价情报本身; 主角用"父亲的旧关系"搪塞。
        "出发前一天，一份没有署名的情报辗转三道中间人送到了你的案头。王后军前锋的驻扎位置、补给线走向、换防节奏——精确到连几位将领的脾气秉性都附在了背面。"

        elena "这份情报……来路可靠吗？精确到这种地步，我反而心里没底。"

        "你告诉她，这是父亲生前留下的一些旧关系。她看了你一眼，没有追问。"

        elena "……好。我拿这份东西重新调整斥候部署。如果是准的，我们能抢到半天先手。"
    else:
    ...
    $ hide_all_chars()
    "小女孩满意地跑走了。她掉了一根头绳在地上——你弯腰捡起来，攥在手心里。"
...
label ch5_preparation:
    ...
    $ hide_all_chars()
    "你环顾四周——奥尔德里克、雷恩、艾琳娜、马修斯。没有人开口，但也没有人低头。"
...
label ch5_counsel_all:
    ...
    aldric "我见过太多权力的争夺。活到最后的那个人，往往不是赢了，而是没站错。"

    if rel_aldric >= 70:
        aldric "但是，我想对您说一句逾越的话——"
        aldric "您的父亲如果还在，他会希望您做正确的事，而不仅仅是做聪明的事。"
        aldric "老领主就是这么没的。但我不后悔跟了他。"
        hide aldric_img
        $ hide_all_chars("player_char_img")
        show player_char_img at left with dissolve
        player "……谢谢你，奥尔德里克。"
    ...
    menu:
        "问他关于父亲的事":
            ...
            if father_murder_mastermind_known:
                ...
                aldric "这事翻出来，有人要解脱，也有人要遭殃。大人心里有数就好。老朽跟到底。"
            else:
    ...
    elena "退而求其次，就是押在快要赢的那一方。但问题是——现在谁都不知道谁会赢。"
    ...
    if dark_lily_joined:
        ...
        lily_master "暗杀，假情报，临阵倒戈——"

        lily_master "这些才是真正决定输赢的东西。"
    ...
    ## --- 决战前夜探监: 地牢里的叔公 (艾德蒙回响G) ---
    ## captured/replaced 两条线艾德蒙都关在主角城堡地牢(replaced 原文明写 1404,
    ## captured 地点原文未写、此处落定不矛盾)。arrested 人在王庭、negotiated 下落
    ## 未写——都不进此场景。与 dark_lily_joined 互斥(那批人这一夜去见影主)。
        ...
        "地牢深处只剩一盏油灯。铁栏后面传来均匀的呼吸声——不像在睡，更像在等。"

        $ hide_all_chars()
        show edmund_img at center with dissolve

        "艾德蒙靠着石墙坐在稻草上，铁百合面罩搁在膝头。他看了你一眼，那双和你一样的棕色眼睛平静得像在看一个迟到的客人。"

        edmund "明天了。"

        "不是问句。你点了点头。"

        edmund "你来这儿不是为了道别。你和你父亲一样——想要答案的时候，什么地方都敢来。"

        if ch3_cult_leader_fate == "captured":
            edmund "你怕的是——万一你明天死了，你父亲那些没有解开的事，会跟着一起埋进土里。"
        else:
            edmund "你怕的是——万一你明天死了，你从我手里夺走的那张网，会在一夜之间散架。"

        "你没有否认。"

        edmund "我给不了你想要的东西。你父亲的事，比你见过的任何一局棋都大。我是其中一个棋子——但只是一个。"

        "油灯的火苗歪了一下。牢房的影子跟着晃了晃。"

        edmund "走吧。去打你的仗。别死在不如你的人手里。"

        "你转身往外走。走到拐角处，身后传来一句很轻的话——"

        edmund "……活着回来。"

        "你没有停步。一个叔公和一个囚徒都有理由说这句话。你不打算分清是哪一个。"

        hide edmund_img with dissolve
...
label ch5_final_night:
    ...
    $ hide_all_chars("captain_img")
    show captain_img at left with dissolve
    captain "领主大人说得对。我头一回上阵的时候，手抖得连盾都举不稳。"
...
label ch5_final_choice:
    ...
    elena "消息是今早送到的，够新。您定了方向，底下的事我来办。"
...
label ending_iron_lord:
    ...
    menu:
        ...
        "围而不攻，逼迫敌军投降":
            $ change_stat("intrigue", 4)
            $ change_stat("loyalty", 2)
            "你没有急着去收割。三面围上，一面留空——你要的不是死战，是溃散。"
            $ hide_all_chars("player_char_img")
            show player_char_img at left with dissolve
...
label ending_shadow_king:
    ...
    "书房里，你摊开地图，把这几个月反复推演过的布局一步步摆上桌面。"
    ...
    if spy_network:
        player "我们的眼线已经渗进了双方阵营。他们调一个营、换一条粮道，我们都知道。"
        hide player_char_img
        $ hide_all_chars("elena_img")
        show elena_img at left with dissolve
        elena "双方的每一步棋，我们都提前知道。这仗还没开打就赢了一半。"
        $ change_stat("intrigue", 5)
    else:
        hide elena_img
        $ hide_all_chars("player_char_img")
        show player_char_img at left with dissolve
        player "虽然我们没有成型的眼线网，但有艾琳娜一个人，抵得上别人十个探子。"
        hide player_char_img
        $ hide_all_chars("elena_img")
        show elena_img at left with dissolve
        elena "少灌迷魂汤。说正事。"
    ...
    player "打下去还有什么意义？再打一天，这片地上就没有活人来庆祝胜利了！"
...
label ending_holy_guardian:
    ...
    bishop "我们要让他们明白——圣母的手比任何剑都长，违逆她的人无处可逃。"
...
label ch5_negotiate_after_clash:
    ...
    menu:
        ...
        "提出一个折衷方案缓和气氛":
            ...
            $ hide_all_chars("player_char_img")
            show player_char_img at left with dissolve
            player "两位，你们的怒气我都看在眼里。但容我说一句——"
            player "谁对谁错，今天先搁下。眼前只有一件事——怎么让外面不再死人。"
            $ hide_all_chars()
    ...
    $ hide_all_chars("player_char_img")
    show player_char_img at left with dissolve
    player "主教大人抬举了。能让村子里的孩子冬天有学堂坐，比什么封号都强。"
...
label ending_peoples_lord:
    ...
    player "我的战场在城墙之内。这里的每个人都要活着看到战争结束。"
    ...
    "流民继续涌入你的领地。几个商人在城堡下租了空屋，卖盐、布和修屋顶用的木料。"

    "你用收容难民时建立的分配系统来管理越来越多的人口。"

    "你把空闲的土地分给了失去家园的农民，让他们重新开始生活。"

    "匠人先修漏雨的屋顶，再填通往村庄的车辙。水渠只挖了最急用的一段。"

    "城墙外多出几排新屋，集市旁仍有人排队领粮。艾登堡活了下来，离富庶还很远。"
...
label ending_truth:
    ...
    if _truth_contested:
        ...
        menu:
            "动用整张情报网——把附和者的底册当场摊开|烧掉多年暗线，换一场干净的胜利" if intrigue >= 60:
                $ change_stat("intrigue", -20)  ## 消耗大轮 Phase 2.2: 把柄当众用掉 = 暗线全部作废
                "你朝艾琳娜看了一眼。她会意，从袖中抽出一叠早已备好的条陈，一份一份传下去。"

                "谁收过王后的年金，谁家的关税少缴了七年，谁的次子在王都的军职是怎么来的——每一份，都对着一个刚才开口的人。"

                "大厅重新安静下来。这一次不是被你压住的——是每个开过口的人都发现，自己的底册也在别人手里。"

                "代价你自己清楚：把柄见了光就是废纸。你经营多年的那张网，今天一口气花完了。"
    ...
    ## 艾德蒙回响H: 清算大潮里叔公/铁刺网的位置。命名铁律: captured/replaced
    ## 可用"艾德蒙", arrested/negotiated 只能"叔公/首领"。真凶依旧不落定。
    if ch3_cult_leader_fate == "captured":
        "铁刺派的案子没有被并入大审判。在王庭的卷宗里，它只是一桩北方地方叛乱。"

        "艾德蒙仍然关在你的地牢里。你本可以在公审中把他交出去。你没有。"

        "原因你自己也说不清楚。"
    elif ch3_cult_leader_fate == "arrested":
        "大审判结束后，有人问起铁刺派首领的处置。王庭的回答是「另案处理」。"

        "你的叔公还关在王都的某间牢房里，仍然没有报上自己的名字。你没有替他报。"
    elif ch3_cult_leader_fate == "negotiated":
        "铁刺派的残余在大审判中被顺带清扫。温和派早已回归暗百合正统，与这场清算再无瓜葛。"

        "你的叔公没有出现在任何一份名单上——不在被告席，不在证人席，也不在旁听席。你不知道他在哪里。"
    elif ch3_cult_leader_fate == "replaced":
        "大审判清算了所有参与阴谋的人。铁刺派的名字从未出现在任何一份判决书上。"

        "艾德蒙仍然关在你的地牢里。那张网仍在运转。你父亲花了二十年拦着自己的叔叔——你花了三个月坐上了同一个位置。"

        "公爵的印章和铁刺派的暗号锁在同一个抽屉里。你还没想好该烧掉哪一个。"
    ...
    "老管家的眼眶湿润了。他在这座城堡里守了三十年，总算等到了这一天。"
    ...
    if elena_romance:
        ...
        elena "……笨蛋。"
        $ hide_all_chars()
        "但她笑了。你很久没见她这样笑——不设防，不算计。"
        "春天来了。新的故事正在开始。"
        hide elena_img with dissolve
    else:
        "你站在花园里，感受着春风拂面。"
        "父亲二十年前没能说出口的真相，你替他说完了。"
        "你的父亲可以安息了。"
        "而你——还有一个王国等着你去守护。"

    "真相大白。正义实现。"

    "路还很长。但至少从今天起，你脚下踩的是实地。"
...
label ending_borgia:
    ...
    $ hide_all_chars()
    "你抬头看夜空。狼毒草的花在月光下泛着深紫色， 跟四十多年前一模一样。"
...
label ending_vassal:
    ...
    ## backlog7: 补 vassal 的 marriage_route 缺口 (批22 五结局镜像的漏网点, fall 有 vassal 没有)
    if marriage_route:
        if marriage_warm:
            "北疆议会在你签字的第七天撤走了全部卫队，只留一句话：婚书作数，盟约冻结。"

            "英格丽没有跟着走。她说：「议会嫁的是盟约。我嫁的不是。」"

            "从此府里多了一个替你把北边来信先读一遍的人。王都的文书你要先看——北疆的信，她先看。"
        else:
            "北疆议会在你签字的第七天撤走了全部卫队。英格丽也走了——盟约既冻，买卖两清。"

            "三个月后她又回来了。议会派的。名义是「照看婚约」，实际是替北疆盯着王都的动向。"

            "你们心里都清楚这是怎么回事。但她住下了，规矩也立起来了——这桩买卖，比看上去的耐用。"
    ...
    if rel_aldric >= 50:
        "奥尔德里克对着账册嘟囔过一句：「协约写的是年贡两次，来的是四次。」王都的规矩就是这样——纸上一套，路上一套。"

    if bridge_mark_seen:
        "头一回收税，你注意到那个收税官的火漆印：三道弧线，叠成一座拱桥。"

        "驿站门楣上那道新鲜的刻痕，在这里对上了。《六卫终录》写桥卫「不知所踪」——不是失踪，是换了主人。死了几百年的第六卫在替王后走她的税路，而你在路上撞见过它的先声。"
    elif knows_eagle_network:
        "头一回收税，你注意到那个收税官的火漆印：三道弧线，叠成一座拱桥——《六卫终录》页脚那个「此记已绝」的记号。"

        "不是失踪。是换了主人。"
    else:
        "带队的收税官佩着一枚旧铜牌，上面的记号你看不懂。他只说这差事他家干了好几代。"
...
label ending_fall:
    ...
    if resist_route:
        "北边那片是男爵的人——正在拔营。会战打输了，他不会为一座守不住的城再赔一支军队。黑狼旗一面接一面地倒下、卷起、装车。"

        "你们并过肩。现在他走他的路。"
    else:
        "北边也有一片， 那是男爵的人。 他们没和王后联手， 也没和你联手， 就这么各走各的路从两个方向压过来。"
    ...
    if elena_romance:
        ...
        "她走前把那份卫队名册烧了。没解释。"
    ## 赛琳(外章 · 南境)。艾琳娜和英格丽是"她几天前走了", 而赛琳**本来就不在艾登堡** ——
    ## 她在千里之外的海上, 根本不知道这座城在烧。
    ## 注意这里有两个 fall: 本段是**主线**的 fall 结局(城破); southern_outcome=="fall" 是
    ## **外章**的 fall(玩家卖了她的港)。两者独立, 可以同时成立。
    elif corsair_romance:
        if southern_outcome == "fall":
            "赛琳在海上——如果渡鸦号还在的话。你卖了她的港，她一个字没回过。"
            "门撞开了。口袋里什么都没有。"
        else:
            "赛琳在海上。她不知道这座城在烧。"
            "门撞开的时候，你摸了一下左边口袋。绳结还在。"
...
label ending_sea:
    ...
    "第一天你一直在等——等骑兵追上来，等有人在身后喊你的名字。什么都没有。"
    ...
    if corsair_romance:
        ...
        if southern_outcome != "fall":
            "你从口袋里掏出那截旧绳头。她教你打的第一个结——揣了四天山路，绳头磨得起了毛。"
