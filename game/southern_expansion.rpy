...
define old_salt = Character("老船工", color="#7a6a5a", image="farmer_rep")
...
define harbor_master = Character("港务官", color="#9a8a6a", image="noble_werner")
define sailor_old = Character("老水手", color="#6a7a7a", image="storyteller")
## 断锚酒馆群像
define tavern_keeper = Character("断锚老板", color="#8a7a5a", image="merchant_karl")
define sea_dog = Character("老汉斯", color="#5a6a7a", image="soldier_generic")
## 第二幕调查配角
define chen_captain = Character("陈船老大", color="#6a5a4a", image="soldier_generic")
...
define royal_admiral = Character("王军主帅", color="#5a4a4a", image="soldier_generic")
...
label southern_dlc_start:
    ...
    "对一个靠海吃饭的港口，封港就是断气。货堆在仓里发霉，鱼烂在舱里发臭，靠工钱过活的人，一天比一天眼神发慌。这样的港口，是干柴，只等一点火星。"
...
label southern_to_tavern:
    ...
    menu:
        "起身道谢，自报家门":
            ...
            "她说得轻巧，可你注意到，整间酒馆里被公会眼线盯着的那桌内陆人，只有你这一桌，她替你挡了。这港口里替人出头的人不多，她偏偏是其中一个——这本身，就说明了点什么。"
    ...
    menu:
        ...
        "先不表态，今晚我要自己去码头看看（谨慎）":
            ...
            $ change_rel("rel_corsair", 5)
            "你接住了哨子。冰凉，边缘有牙印——被人紧张地咬过很多次。一个在港口里横着走的渡鸦船长，也有过怕得只能咬着哨子等人来的时候。她把这东西给你，给的不只是一条退路，还有一点她大概自己都没察觉的、对你的信。"
...
label southern_act3:
    ...
    "屋里的气氛松了一丝。可你心里清楚，路是有了，难的在后头——要让斗了三十年的公会和船主，在一夜之间拧成一股绳，比挡住那支舰队还难。刀枪能用沉船暗礁挡，人心里那道沟，拿什么填？"
...
label ending_southern_outwit:
    ...
    pause 1.0
    centered "{size=+3}潮汐港至今不知道，它是被一个外乡人，从棋盘上轻轻拿走的。{/size}"
    pause 2.0
...
label ending_southern_fall:
    ...
    if corsair_romance:
        ...
        "碑文里没有费舍尔，没有海雀号，没有那场子虚乌有的'走私'。赢家写的史书，本来就不记这些。"
