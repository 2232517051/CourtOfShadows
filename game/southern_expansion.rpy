...
## 属性基线留在这里。批39 曾说这条"与并入主线前的体验逐字一致" —— **那句现在不成立了**,
## 后续几批改了门槛(批41)、降了三个结局的政治当量并改名(批43)、换掉了收尾包装(批49)。
## 现在两个入口共用同一份正文, 差别只有: 独立入口跑下面这段属性基线, 主线入口不跑。
label southern_arc_standalone:

    ## 外传设定在领主治下，主角已立足 —— 给一个已成势的属性基线，而非主线开局的新丁数值。
    ##
    ## 【2026-07-16 复核】原注释在这里写的理由是"否则 intrigue/reputation 门槛选项全为死枝"
    ## —— **该理由已不成立**: 批41 把六处门槛从"属性"改挂到了弧内累加器(port_insight 0-6 /
    ## evidence_count 0-7 / southern_first_impression / rel_corsair), 属性只当捷径 OR。
    ## 所以这个基线现在**不再是门槛可达性的必要条件**, 它只剩"外传主角是个已成势的领主"
    ## 这一层设定意味。
    ##
    ## 复核实测(基线 intrigue45/reputation50, 弧内零投入): 六处门槛里 2 处比重挂前松了 ——
    ##   :1277 破局点自己想到  旧不通过 → 新通过 (纯风味分支: 是他自己想到还是 NPC 点破)
    ##   :1348 请附(vassal)   旧不通过 → 新通过 (多开一个结局选项)
    ## 另外查证 3 条证据后的智取(outwit): 旧不通过(要 intrigue>=60) → 新通过。
    ## **判断: 接受, 不调**。理由: ①新门槛的设计原则是"主门槛给你在港里干了什么, 属性只当
    ## 捷径", 而"已成势的领主有资格向王廷请附"正是这条捷径该有的样子; ②:1277 只是换个人
    ## 说同一句话, 不门控内容; ③智取从"要把属性刷到60"变成"要认真查证", 是更好的门控。
    ## 要收紧的话, 正确做法是**调低这里的基线**(理由已消失), 而不是给门槛加入口判断 ——
    ## 但那会改变独立入口的既有手感, 属于需要真机 playtest 定的事, 不在本轮擅自动。
    ##
...
label southern_arc:
    ...
    "盐路断不得——不光是因为盐。一个连王廷都管不着的自由港闹出这么大的乱子，背后未必只是两伙生意人抢地盘那么简单。"
    ...
    "你这辈子没见过这么多船。船挨着船，桅杆挨着桅杆，密得看不见水面。"
...
label southern_explore_market:
    ...
    menu:
        "潮汐港三百年没有王法，是怎么撑下来的？":
            ...
            fishmonger "靠海呗。这地方是个口子，什么货都打这过。有钱赚就有人来，有人来就有人争。"

            fishmonger "外头眼红这港口的多了去了，可谁都吞不下——你刚张嘴，旁边几个就掐你脖子。三百年了，谁也没占着。"
...
label southern_explore_guild:
    ...
    $ port_insight += 1
    scene bg tideport_harbor with dissolve
    "金锚公会的大楼占着港口最好的地段，三层石砌，比周围的木屋高出一截。门口两尊石锚，门楣上鎏金的徽记，老远就晃眼。"
...
label southern_to_tavern:
    ...
    menu:
        "公会和船主，你觉得谁占理？":
            $ change_stat("intrigue", 1)
            $ broker_rumor_heard = True
            tavern_keeper "这港口讲理的没几个。公会攥着钱和章子，说了算；船主攥着刀和船，不买账。两条狗抢一根骨头，谁也不松嘴。"
    ...
    "她走进来的那一刻，满屋子掷骰子的、谈买卖的、喝闷酒的，声音都低了半度。这种安静，不是怕，是敬——在这片认拳头的港口，能让人不动声色就矮三分的，没几个。"
    ...
    menu:
        ...
        "不动声色，先看她想做什么":
            ...
            "她打量了你一会儿，像在琢磨你到底是什么路数。能稳稳坐在这种地方、还敢这么跟她说话的内陆人，她大概没见过几个。"
    ...
    corsair "他们控制码头、定价、放贷。老汉斯跟你说的那些全是真的——可他说的还只是皮毛。"
    ...
    guild_master "站在公会这边。这扇门今晚开着——过了今晚，就不好说了。"
    ...
    corsair "维斯帕给你画的那些大饼——商号、低息、优先放行，听着是甜。可你想想，他凭什么给？还不是因为整个港口的买卖都从他手里过。哪天你不听话了，他收回去比给出来还快。"
...
label ending_southern_free:
    ...
    "舰队退到海平线以下那天，港口的铁链解开了。闷了多日的商船一条接一条驶出港口，桅杆上重新挂满了帆。"
...
label ending_southern_ruler:
    ...
    if corsair_romance and southern_faction == "pirates":
        ...
        ## 原此处为"往后的潮汐港，再没有公会和船主的火并——只剩一个声音说了算，你的。"
        ## 批43 降级 ruler 时改了它左右两句(上一句"临时的代管人…之后是留是走，不由你定"、
        ## 下一句"「代管」两个字后面跟着的是「到开春」")却独独跳过它 —— 玩家会在同屏内连读:
        ## 权位不由你定 → 权位独你一人且往后如此 → 权位到开春为止。
        ## "只剩一个声音说了算，你的"正是降级要删掉的那个设定, 故删除本句。

        "码头上的血迹洗干净了，港里消停了几天。可你心里清楚，「代管」两个字后面跟着的是「到开春」。"
...
label ending_southern_fall:
    ...
    if corsair_romance:
        ...
        "往后站在城头，有时会想起一个抱孩子的女人，和一个说'我们是清白的'的硬汉。可也只是想想。日子还得过。"
...
label southern_arc_return:
    ...
    ## 职责只有一条: 把玩家从潮汐港送回艾登堡, 接第二章。
    ## 结局的定性/感慨/悬念, 结局正文和标题卡已经说完了 —— coda 一个字都不再说。
    ## (初版三段 coda 把结局正文刚说过的话又说一遍: vassal 线玩家要连读三遍"你分不清那番
    ##  话管没管用", ruler 的 coda 与结局正文里"代管港务到开春"几乎逐字重复。那正是默然
    ##  投诉的"经常进入同一个以前经历过的剧情"。)
    ##
    ## 地理: 艾登堡是内陆领地(CANON.md 二·B), 主角**不能坐船到家**。回程是去程的逆序:
    ## 潮汐港 → 海船 → 王国南边的海港城下船 → 四天陆路翻两道山 → 艾登堡。
    ## (初版五段全写成"码头/栈桥/靠岸/下船", 与 southern:265 的去程和 chapter1_expansion:878
    ##  的"可惜艾登堡不靠海"打架。已下沉为 Tools/scan_canon.py 的 GEO_PATTERNS。)
    if southern_outcome == "free":
        "你站在甲板上看潮汐港的灯火一点点缩成海平面上的亮斑。没人来送——也没人有义务送。"
        "港口签了新约，盐路照旧通行。你没在新约上留名，也没挂旗。"
        "回程的海风比来时顺。你在船舱里补了好几天觉。"
        "翻过最后一道山梁，艾登堡的城墙从晨雾里露出来。奥尔德里克牵着马等在官道尽头，开口第一句是问你吃了没。"

    elif southern_outcome == "ruler":
        "回程的船逆风，比来时慢了两天。你在船舱里翻了一路账册，潮汐港去年的关税比艾登堡全年的税收还多。"
        "在南边港城下了船，走了四天山路。快到艾登堡时，路边水洼结了薄冰，踩上去嘎吱响。"
        ...
        "进了艾登堡的城门，街边仓房敞着门，里头码着刚到的盐包。盐路通了，够了。"
        "奥尔德里克从账房出来迎你。他没问潮汐港的事，先递过来一摞秋税的账本。"
        ...
        "到艾登堡那天下着小雨。奥尔德里克打伞等在院子里，看了你一眼，什么都没说。"
...
label southern_act2:
    ...
    "你正要走，听见货堆后头有动静。你和奥尔德里克对视一眼，他悄悄按住了腰间的短刀。"
