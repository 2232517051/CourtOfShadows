...
label chapter4_start:
    call new_run_bootstrap from _call_new_run_bootstrap_chapter4
    ...
    if not dark_lily_joined:
        ...
        ## 艾德蒙回响A(接管线): 城墙上没挥手的沉默 = ch3 接管后 rel_aldric-10 的演出层
        if iron_thorn_controlled:
            "你忽然想到——如果那些老兵知道你在城堡地牢里关着什么人，知道你手里攥着一张什么样的网，他们恐怕不会只用「复杂」两个字。"

            "奥尔德里克全都知道。这也许就是他没有挥手的原因。"
    else:
    ...
    if father_letters_found:
        ...
        ## 艾德蒙回响B(四种处置通用): 走过支线的玩家对这个问题必然联想到被除名的叔公
        if ch3_cult_leader_fate:
            "你脑子里立刻浮出一个人——祖父那个被家族除名的弟弟。他拉走了铁刺派，父亲拦了他一辈子。也许这些信，就是拦的方式。"

            "但信里的语气不像在拦。更像两个人在合力盯着同一个东西——暗百合。"

            "你把信纸翻过来，对着火光看了看背面。什么也没有。答案还在更深的地方。"
        $ change_stat("intrigue", 5)
...
label ch4_palace:
    ...
    if marriage_proposal_open or marriage_route:
        ...
        show ingrid_img at left with dissolve
        ingrid "很好。我也只答应谈。"
        ingrid "母亲要盐路重新通。议会要撑到开春。你要北境别站到你的敌人那边。"
        ingrid "盐路、粮食、出兵，都可以让书记写进盟书。至于我嫁不嫁——"
        ingrid "母亲把我送到这里，是让我自己开口。"
        ...
        menu:
            "接受婚约，除此之外不作承诺":
                ...
                player "我答应婚约。艾登堡重开盐路，拨出粮食；北疆议会若有兵可调，就替我守住北面。除此之外，我们不必假装。"
                hide player_char_img
                $ hide_all_chars("ingrid_img")
                show ingrid_img at left with dissolve
                ingrid "好。人前我会尽到妻子的礼数；关上门，我们各过各的。盟书上写多少，就给多少。别因为我住进来多添一袋粮。"
            ...
            "接受婚约，问英格丽本人的打算":
                ...
                $ change_stat("intrigue", 2)
                player "婚约我答应。只是我们以后要住在同一座城堡，我不想只在盟书上认识你。"
                player "你若也愿意，就从今天开始。"
                $ hide_all_chars("ingrid_img")
                show ingrid_img at left with dissolve
                "英格丽没有立刻回答。她把桌上的盟约草案翻到空白的一面。"
                ingrid "我愿意。先把盐路的事办好。等到了艾登堡，你再慢慢问。"
            ...
            "拒绝婚约，结束商谈":
                ...
                player "婚事我不答应。盐路、粮食和出兵若还要谈，日后另派使者来。今天只把这件事说清楚。"
                hide player_char_img
                $ hide_all_chars("ingrid_img")
                show ingrid_img at left with dissolve
                ingrid "母亲不会喜欢这个答复。"
                ingrid "这是我来听的答复。我收下。婚事的提议，到此为止。"
...
label ch4_throne:
    ...
    if ch3_cult_leader_fate == "arrested":
        queen "对了，你报上来的那桩铁刺派案子——审讯官说那个首领到现在不肯报上真名。三次提审，一个字没吐。"

        queen "宁肯烂在牢里也不说自己是谁——是在替什么人守秘密呢，还是怕报了名之后……有人睡不着觉？"

        "王后说完最后一个字，目光在你脸上多停了一瞬——像是在等你的表情出错。"
    ...
    "你下意识地看了一眼主位。王后确实在微笑，但那种笑容让你不寒而栗。"
...
label ch4_elena:
    ...
    if not elena_identity_exposed_known:
        ...
        ## 代词修正: if 分支说的是"你的父亲"(卡尔)识破并引荐, else 复述时误写成"我父亲"
...
label ch4_end:
    ...
    ## 艾德蒙回响E: 地图上属于铁刺派的那一角, 按处置方式各有一笔
    if ch3_cult_leader_fate == "captured":
        "你在铁刺派三处据点的旧址上画了叉——它们离艾登堡粮道近得让你后怕。"
    elif ch3_cult_leader_fate == "arrested":
        "你在铁刺派旧址上标了王庭的徽记——那些地方现在归皇家调查团管辖，你连靠近的权限都没有。"
    elif ch3_cult_leader_fate == "negotiated":
        "你在温和派回归的几个据点旁画了虚线——他们说已经回归正统，但你画不出一条实线来。"
    elif ch3_cult_leader_fate == "replaced":
        "你的笔在铁刺派据点的位置上空悬了一下，然后跳过了它们——有些东西不能落在任何一张纸上。"
