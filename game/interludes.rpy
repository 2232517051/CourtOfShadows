...
label interlude_ch1_ch2:
    ...
    ## 外章 · 南境: 南下的玩家刚在船上颠了小一个月, "你刚刚度过了第一次真正考验"读起来是穿越。
    ## (附注: 本段本就有既存时序错位 —— 它在 chapter2.rpy:46 被 call, 即"一个月过去了"之后,
    ##  内容却写第一章当晚。不是外章引入的, 本次不修, 但变体不该让它更糟。)
    if southern_outcome not in ("none", "delegated"):
        "你推开书房的门。你的椅子上积了一层灰——一个月没人坐过。"

        "文书堆成了小山。税收账册、边境巡逻报告、来自王都的密函。奥尔德里克挑走了急件，剩下的全搁着等你。"

        "你坐下来。船上晃了快一个月，总算有张不晃的桌子。"
    else:
    ...
    if father_death_known:
        "父亲的死因还是一团雾。你手里那几条线索拼在一起，隐隐指向一个让你后脊发凉的方向。"
    else:
...
label interlude_ch4_ch5:
    ...
    menu:
        ...
        "当面拒绝——这超越了王后的权限。":
            ...
            $ hide_all_chars("player_char_img")
            show player_char_img at left with dissolve
            player "根据先王颁布的《领地自治法》，领地内政由领主裁处，王室无权越权干涉。"
            $ hide_all_chars()
            "特使的脸色变了。"
            "「领主大人，我劝您——」"
            $ hide_all_chars("player_char_img")
    ...
    aldric "修道院报告说……老仆人和守墓人偶尔在月夜去您父亲的墓前祭奠。最近去得更勤了——他们说，是想老领主了。"
    ...
    "王都还在角力，王后已经把手伸到了家门口，领地里的暗火也压不下去了——"
...
label interlude_ch1_ch2_dream:
    ...
    "橡木书架上摆满了皮革封面的典籍，空气中弥漫着墨水和松脂的气息。连桌角那个磨出毛边的墨水瓶都没挪过位置。"
...
label interlude_ch4_ch5_council:
    ...
    menu:
        ...
        "民生优先——先解决内部矛盾，赢得民心。军心来自民心。":
            ...
            "不许诺做不到的事，但让他们知道——领主没有忘记他们。"

            "等村长拉着你的手说「开口就是」那一刻，你觉得比加固一段城墙还值。"
        ...
        "经济优先——没有钱粮，军队和民心都是空话。":
            ...
            "同时，你秘密派人在几处要紧的地方安插了耳目——你需要知道敌人的一举一动。"
...
label _interlude_marcus_returns:
    ...
    "你独自从地窖回书房——来来回回走了不知多少趟，几乎闭着眼都能走。"
