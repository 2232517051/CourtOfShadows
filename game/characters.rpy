...
## 艾德蒙 — 铁刺派首领, 被家族除名。canon(2026-07-29 用户拍板): 康拉德(祖父)之弟,
## 即主角亚瑟的叔祖父、卡尔的叔叔; 除名在 1298 年前, 故前传无此人
...
default father_poison_method_known = False       # 已确认暮色之露是父亲死因
...
init python:
    ...
    achievement_data = {
        ...
        "sea_ending":       ("南渡", "达成南渡结局", False, "去过南方的人才知道海有多远"),
        "completionist":    ("全能领主", "解锁所有九个结局", False, "达成全部九个结局"),
        "max_stat":         ("登峰造极", "任一属性达到100", False, "将任一属性提升至最高"),
        ## 南境游记 DLC
        "southern_act1":    ("初探潮汐港", "在外章中作出你的抉择", False, "进入外章并走完第一幕"),
        ## 本表必须与 southern_expansion.rpy 的 _southern_ending_info 保持同口径 ——
        ## 两张表都描述同一批结局, 一张是结局路线图/图鉴, 一张是成就弹窗(effects.rpy:737)
        ## 与成就殿堂(gallery.rpy:388)。批43 降级三个结局时只改了 _southern_ending_info,
        ## 漏了这张 —— 后果: 标题卡「借来的港·开春以后的事文书上没写」淡出 2 秒, 成就弹窗
        ## 紧接着断言「港口新主·掌控潮汐港」; vassal 更锋利, 标题卡刚说"你那番话管没管用,
        ## 至今没人告诉你", 弹窗就说"促成潮汐港归附王廷" —— 把降级专门要删的因果又讲一遍。
        ## 顺带: 本表的 hint 此前还留着旧的军队称谓(批40 已统一为"王室军队")。
        "southern_free":    ("自由港", "让潮汐港作为自由港存续", False, "促成两派联合，逼退王室军队"),
        "southern_ruler":   ("借来的港", "代管潮汐港港务，管到开春", False, "倒向一方，代管港务到开春"),
        "southern_fall":    ("潮汐港陷落", "潮汐港落入王廷之手", False, "与王室军队私下交易，出卖港口"),
        "southern_lover":   ("渡鸦与你", "与渡鸦船长赛琳缔结情谊", True, "渡 隐藏成就"),
        "southern_outwit":  ("扔出去的信", "把证据投进黑暗，舰队撤了", True, "谋 隐藏成就"),
        "southern_vassal":  ("该来的旗", "潮汐港归附王廷，而诏书未必因你而来", False, "归附王廷，诏书未必因你而来"),
        ## 隐藏成就 — 未解锁前不显示名称和描述
