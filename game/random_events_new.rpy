...
init python:
    ...
    _scene_events = [
        ...
        ## 决斗挑战: 需要玩家已在贵族圈里露过面; max_chapter=4 —— 第五章王后已集结王室军队、
        ## 男爵举旗反叛, 一个银甲骑士骑着高头大马来城堡下战书(:1002)在那个语境里出戏。
        ("re_duel_challenge",   "re_duel_met",          2, 4),
        ## ── re_harvest 不接入 ──
        ## 它自带一整年: 开场":1251 秋天到了。田野里的麦穗沉甸甸地低着头", 四条出口全部演完
        ## 整个冬天(:1318"这个冬天, 粮仓是满的" / :1367"到春天来临的时候" / :1423"开春之后")。
        ## 而主线是 深秋(script.rpy:97"收割后的麦田, 金黄的麦茬") → 初冬 → 三月初春, 约 4-5 个月
        ## (CANON.md 三·B), **起点就在秋收之后**。所以这个事件塞进任何一章都对撞:
        ##   ch5 最惨 —— chapter5.rpy:43 调用, 下一行 :45 就是"开春了。冰雪消融", 一章过两次春天
        ##   ch1 —— 调用点距"你度过了成为领主的第一周"仅 17 行, 整段秋冬春塞进第一周,
        ##          且"麦穗沉甸甸"与开场"收割后的麦田"直接打架
        ##   ch2 —— 紧邻 gov_famine_crisis(旱灾"今年的收成最多只有往年的三成"), 抽中就是
        ##          管家前脚报旱灾、后脚报"今年是丰收年, 小麦增产三成"
        ## 收窄窗口救不了它 —— 它的开场比主线起点还早一个季节。要接入得先重写成"收成报告"
        ## 量级(去掉跨季演出), 那是新写正文, 不在本轮范围。
        ## 单周目因此见 5/8 而非 5/9。
    ]
